...
from src.bert_model.process_walks_gcn import Processing_GCN_Walks, get_normalized_masked_ids
...
def setup_pretraining_input(args, attr_graph, context_gen, data_path):
    ...
    split_file = {task: os.path.join(data_path, args.data_name+'_pretrain_'+task+'.txt')
                for task in tasks}
    if os.path.exists(split_file['train']):
        ...
        all_walks = context_gen.get_pretrain_subgraphs(data_path, \
            args.data_name, args.beam_width, int(args.max_length), args.walk_type)
        print('\n split data to train/validate/test and save the files ...')
        walk_train, walk_validate, walk_test = split_data(all_walks, data_path, args.data_name)
    ...
    for task in tasks:
        if task == 'train':
            ...
            walk_data = walk_test
        cnt = create_batches(walk_data, data_path, args.data_name, task, args.batch_size)
        num_batches[task] = cnt
    print('number of batches for pre-training: ', num_batches)
    return num_batches

def run_pretraining(args, attr_graph, no_batches, pretrained_node_embedding, ent2id, rel2id):
    ...
    batch_input_file = os.path.join(data_path, 
        args.data_name+'_batch_input.pickled')
    if os.path.exists(batch_input_file):
        ...
        for task in tasks:
            ...
            for batch_id in range(no_batches[task]):
                subgraphs_list, all_nodes, masked_nodes, masked_postion = \
    ...
    for epoch in range(int(args.n_epochs)):
        ...
        for batch_id in range(no_batches['train']):
            ...
            if(args.get_bert_encoder_embeddings):
                logits_lm, _ = \
                graph_bert(subgraphs_list, all_nodes, masked_postion, masked_nodes)
                #node_embeddings[subgraphs_list] = bert_subgraph_embedding_output 
            else:
                logits_lm = graph_bert(subgraphs_list, all_nodes, masked_postion, masked_nodes)
            #print('check ====', logits_lm.size(), masked_nodes_id.size())
            try:
                normalized_masked_nodes_id = get_normalized_masked_ids(masked_nodes, ent2id)
            except:
                normalized_masked_nodes_id = masked_nodes
            # print(logits_lm.size(), normalized_masked_nodes_id.size())
            loss = criterion(logits_lm.transpose(1, 2), normalized_masked_nodes_id.cuda())
            ...
            if (batch_id % args.checkpoint == 0 and batch_id > 0) or batch_id == no_batches['train']-1:
                print()
                ...
                with torch.no_grad():
                    ...
                    for batch_dev_id in range(no_batches['validate']):
                        ...
                        if(args.get_bert_encoder_embeddings):
                            logits_lm, _ = \
                                graph_bert(subgraphs_list, all_nodes, 
                                masked_postion, masked_nodes)
                            #node_embeddings[subgraphs_list] = bert_subgraph_embedding_output
                        else:
                        ...
                        try:
                            normalized_masked_nodes_id = \
                                get_normalized_masked_ids(masked_nodes, ent2id)
                        except:
                            normalized_masked_nodes_id = masked_nodes
                        #print(normalized_masked_nodes_id.size())
                        loss = criterion(logits_lm.transpose(1, 2), \
                            normalized_masked_nodes_id.cuda())
                ...
                if loss_dev_avg < loss_dev_min:
                    ...
                    fmodel = open(os.path.join(
                        out_dir, 'bert_'+str(epoch)+'.model'), 'wb')
    ...
    fbest = open(os.path.join(
        out_dir, 'best_epoch_id.txt'), 'w')
    ...
    with torch.no_grad():
        for batch_test_id in range(no_batches['test']):
            subgraphs_list, all_nodes, masked_nodes, masked_postion =\
                batch_input['test'][batch_dev_id]

            if(args.get_bert_encoder_embeddings):
                logits, bert_subgraph_embedding_output = \
                graph_bert(subgraphs_list, all_nodes, masked_postion, masked_nodes)
                ...
                logits = graph_bert(subgraphs_list, all_nodes, \
                    masked_postion, masked_nodes)
