...
from src.wide_deep_model.InputEncoder import NodeEncoder
...
def norm_graph(node_id_map, edge_id_map, edge_list):
    norm_edge_list = []
    for e in edge_list:
        norm_edge_list.append((get_norm_id(node_id_map, e[0]),\
...
def norm_context_subgraphs_for_gcn_encoding(subgraph_list):
    ...
    for i in range(num_subgraphs):
        node_id_map = dict()
        edge_type_map = dict()
        norm_subgraph_list.append(norm_graph(node_id_map, edge_type_map, \
                subgraph_list[i]))
...
class GCNGraphEncoder(torch.nn.Module):
    def __init__(self, 
        G, pretrained_node_embedding_tensor,
        is_pre_trained,
        base_embedding_dim,
        max_length,
        ent2id,
        ...
        self.node_embedding = NodeEncoder(
            base_embedding_dim, self.no_nodes,
            pretrained_node_embedding_tensor, is_pre_trained)
        self.relation_embed = \
            torch.nn.Embedding(self.no_relations, base_embedding_dim)
            # torch.nn.Embedding(len(self.rel2id), base_embedding_dim) #udpated for KGAT data
    ...
    def forward(self, subgraphs_list, masked_nodes):
        ...
        node_emb = torch.zeros(num_subgraphs, self.max_length+1,\
            self.base_embedding_dim)
        relation_emb = torch.zeros(num_subgraphs, max_relations,\
            self.base_embedding_dim)
        ...
        for ii in range(len(subgraphs_list)):
            ...
            for node_id, norm_node_id in node_id_map.items():
                if node_id not in masked_set: #used to ignore the masked nodes
                    ...
                    node_emb[ii][norm_node_id] = \
                        self.node_embedding(normalized_node_id)
            ...
            for relation_id, norm_relation_id in edge_type_map.items():
                ...
                relation_emb[ii][norm_relation_id] = \
                    self.relation_embed(tmp)
        ...
        for token in self.special_tokens:
            ...
            special_tokens_embed[self.special_tokens[token]+self.no_nodes] = \
                {'token': token, 'embed': tmp_embed}

        return norm_subgraph_list, node_emb, relation_emb,\
            batch_id_maps, special_tokens_embed