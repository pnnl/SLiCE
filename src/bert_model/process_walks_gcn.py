...
def get_normalized_masked_ids(masked_node_ids, ent2id):
    ...
    for batch_node_ids in masked_node_ids:
        normalized_node_ids_batch = [ent2id[str(x.cpu().numpy().tolist())] \
            for x in batch_node_ids]
...
class Processing_GCN_Walks:
    def __init__(self, nodeid2rowid, relations, 
        n_pred, max_length, max_pred):
    ...
    def getSpecialTokens(self):
        ...
        for ii in range(len(special_tokens_set)):
    ...
    def pad_max_seq_len(self, subgraphs_list, max_seq_len):
        ...
        for subgraph in subgraphs_list:
            ...
            for nn in range(max_seq_len-subgraph_actual_len):
