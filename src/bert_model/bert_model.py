...
class GraphBERT(torch.nn.Module):
    ...
    def gcn_out2bert_input(self, node_emb, batch_id_maps, \
    ...
    def forward(self, subgraphs_list, all_nodes, masked_pos, masked_nodes):
        norm_subgraph_list, node_emb, relation_emb, \
        ...
        node_emb = self.gcn_out2bert_input(node_emb, batch_id_maps, \
            all_nodes, special_tokens_embed)
        ...
        if(self.fine_tuning_layer):
            ...
            h_masked = torch.gather(output, 1, masked_pos.cuda()) # masking position [batch_size, len, d_model]
...
class FinetuneLayer(torch.nn.Module):
    def __init__(self, d_model, ft_d_ff, ft_layer, ft_drop_rate,
        attr_graph, ft_input_option, n_layers):
            ...
            for ii in range(start_layer, no_layers):
                ...
                if self.ft_input_option == 'last4_cat':
                    try:
                        ...
                        destination_embedding = torch.cat((destination_embedding, \
                            destination_embed), 2)
                    except:
                    ...
                    try:
                        ...
                        destination_embedding = torch.add(destination_embedding, 1,\
                            destination_embed)
                    except:
                        source_embedding = source_embed
                        destination_embedding = destination_embed   
