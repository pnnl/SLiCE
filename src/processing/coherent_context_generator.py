...
class ContextGenerator:
    ...
    def get_random_k_nbrs(self, nodeid, exclude_list, k):
        ...
        valid_nbrs = all_nbrs
        if (exclude_list == None or len(exclude_list) == 0):
            valid_nbrs = all_nbrs
        else:
    ...
    def select_coherent_nbrs(self, source, target, k):
        ...
        if k >= 0 and len(all_nbrs) >= k:
            all_nbrs_score = [self.path_metrics.get_similarity_score(x, target) for x
                              in all_nbrs]
    ...
    def gen_random_walk_context(self, beam_width, max_num_edges,
                                walk_type, valid_patterns = []):
        """
    ...
    def bfs_beam(self,source, beam_width):
        ...
        for src_nbr in source_nbrs:
            src_nbr_hop2_cands = self.get_random_k_nbrs(src_nbr, exclude_list=[source], k=beam_width)
    ...
    def get_pretrain_subgraphs(self, data_path, data_name, beam_width,
                               max_num_edges, walk_type,
                               mode='pretrain'):
    ...
    def get_finetune_subgraphs(self, train_edges, valid_edges, test_edges,
                               beam_width, max_seq_len, option, valid_patterns =
                              []):
        """
        ...
        to_be_removed_source_target_pairs = [(x[1], x[2]) for x in
                                             to_be_removed_edges]
        self.attr_graph.G.remove_edges_from(to_be_removed_source_target_pairs)
        walks_by_task_dict = dict()
        for task, task_edges in zip(['train', 'valid', 'test'],
                                         [train_edges, valid_edges,
                                          test_edges]):
            task_subgraphs = []
            for edge in task_edges:
                ...
                source_target_contexts = self.get_selective_context(source, target,
                                                          beam_width,
                                                          max_seq_len-1,
                                                                    option,
                                                                    valid_patterns)
    ...
    def get_pattern_path(self, source, target, beam_width, max_seq_len, pattern, currpath):
        ...
        for n in source_nbrs:
            if (n != target and n not in currpath):
                ...
                nbr_paths = self.get_pattern_path(new_source, target,
    ...
    def get_selective_context(self, source, target, beam_width, max_seq_len,
                              option, valid_patterns = []):
        """
        ...
        if(option == 'shortest'):
            ...
            for pattern in valid_patterns:
                ...
                paths = self.get_pattern_path(source, target, beam_width, len(pattern)+1, pattern, currpath=[source])
                if(len(paths) >= 1):
                    path = paths[0]
                    path_with_edge_label = []
                    for i in range(len(path)-1):
                        u = path[i]
                        v = path[i+1]
                        path_with_edge_label.append((u, v,
                                                     G.edges[u,v]['label']))
            ...
            node_list_paths = self.get_random_paths_between_nodes(source, target, beam_width,
                                                   max_seq_len, currpath=[],
                                                                  selection=option)
    ...
    def get_random_paths_between_nodes(self, source, target,  beam_width,
                                       max_seq_len, currpath, selection):
        """
        ...
        for n in source_nbrs:
            if (n != target and n not in currpath):
                ...
                new_path = list(currpath)
                new_path.append(new_source)
                nbr_paths = self.get_random_paths_between_nodes(new_source,
                                                                target,
                                                                beam_width,
                                                                max_seq_len,
                                                                new_path,
