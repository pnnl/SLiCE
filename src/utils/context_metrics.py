...
class PathMetrics():
    ...
    def __init__(self, pretrained_node_embedding, G, ent2id, \
        d_model, ft_d_ff, ft_layer, ft_drop_rate, attr_graph, ft_input_option, n_layers):
        ...
        self.ft_linear = FinetuneLayer(d_model, ft_d_ff, 
            ft_layer, ft_drop_rate, attr_graph, ft_input_option, n_layers)

    def update_batch_graphbert(self, subgraph_batch, scores, labels, \
        graphbert_output, nodes_seq, train_stage):
        for ii in range(len(subgraph_batch)):
            ...
            coherence, variance, min_score, metapath =\
                self.get_path_metrics_graphbert(path, embedding)
    ...
    def update_batch(self, subgraph_batch, labels):
        for ii in range(len(subgraph_batch)):
            ...
            coherence, variance, min_score, metapath = self.get_path_metrics(source, target, path)
            tmp_dict = {'label': label,
                        'relation': relation,
                        'score': pred_score,
                        'path': path,
