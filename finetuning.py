...
from src.bert_model.bert_model import *
...
from src.utils.utils import show_progress, load_pickle
...
from src.utils.context_metrics import *
...
def setup_finetuning_input(args, attr_graph, context_gen):
    ...
    if(os.path.exists(finetune_path + "/finetune_walks.txt")):
        finetune_walks_per_task = json.load(
            open(finetune_path + "/finetune_walks.txt"))
    else:
        train_edges, valid_edges, test_edges= attr_graph.generate_link_prediction_dataset(\
        ...
        finetune_walks_per_task = context_gen.get_finetune_subgraphs(\
            ...
            int(args.beam_width), int(args.max_length), args.path_option)
        json.dump(finetune_walks_per_task, 
            open(finetune_path + "/finetune_walks.txt", "w"))

        print("Number of train, valid, test", \
            len(train_edges), len(valid_edges), len(test_edges))
    num_batches = CreateFinetuneBatches2(finetune_walks_per_task, finetune_path,
            args.data_name, args.ft_batch_size)
    print('No. of batches for finetuning:', num_batches)
    
    return num_batches


def run_finetuning_wkfl2(args, attr_graph, no_batches, pretrained_node_embedding, ent2id, rel2id, epoch=-1):
    ...
    walk_processor = Processing_GCN_Walks(nodeid2rowid, relations, 
        args.n_pred, int(args.max_length), args.max_pred)

    #process minibatch for finetune(ft)
    print('\n processing minibaches for finetuning:')
    ft_batch_input_file = os.path.join(finetune_path, \
        args.data_name+'_ft_batch_input.pickled')
    ...
    graph_bert = GraphBERT(int(args.n_layers), int(args.d_model), 
        args.d_k, args.d_v, args.d_ff, int(args.n_heads), attr_graph, 
        pretrained_node_embedding_tensor, args.is_pre_trained,
        args.base_embedding_dim,
        int(args.max_length), args.num_gcn_layers, 
        args.node_edge_composition_func, 
        args.gcn_option, args.get_bert_encoder_embeddings,
        ent2id, rel2id)
    ...
    pred_data = {}
    true_data = {}
    path_metrics_graphbert_pre = PathMetrics(None, attr_graph.G, ent2id,
        args.d_model, args.ft_d_ff, args.ft_layer, args.ft_drop_rate, 
        attr_graph, args.ft_input_option, args.n_layers)
    with torch.no_grad():
        for data_type in ['valid', 'test']:
            ...
            for batch_id in range(no_batches[data_type]):
                ...
                subgraphs, all_nodes, labels  = \
                    ft_batch_input[data_type][batch_id]
                    
                masked_pos = torch.randn(args.batch_size, 2)
                masked_nodes = Variable(torch.LongTensor([[] for ii in range(args.ft_batch_size)]))
                output, layer_output, _ = graph_bert(subgraphs, all_nodes, masked_pos, masked_nodes)
        ...
        data = json.dump(path_dict, open(outdir+'path_metrics_graphbert_pre.json', 'w'), indent=4)

    return pred_data, true_data


def run_finetuning_wkfl3(args, attr_graph, no_batches, pretrained_node_embedding, ent2id, rel2id):
    ...
    ft_linear = FinetuneLayer(args.d_model, args.ft_d_ff, 
        args.ft_layer, args.ft_drop_rate, attr_graph, args.ft_input_option, args.n_layers)
    ...
    for epoch in range(int(args.ft_n_epochs)):
        ...
        for batch_id in range(no_batches['train']):
            ...
            with torch.no_grad():
                #random mask
                masked_pos = torch.randn(args.batch_size, 2)
                masked_nodes = Variable(torch.LongTensor(\
                    [[] for ii in range(args.ft_batch_size)]))
                output, layer_output, _ = \
                    graph_bert(subgraphs, all_nodes, masked_pos, masked_nodes)
            ...
            message = 'Loss: {}, AvgLoss: {}{}'.format(
                ...
                np.around(np.average(loss_arr).tolist(), 4), ' '*10)
            show_progress(ccnt, min(int(args.ft_checkpoint), no_batches['train']), message)

            if (batch_id % int(args.ft_checkpoint) == 0 and batch_id > 0) \
                or batch_id == no_batches['train']-1:

                print()
                print('Batch: {}, Loss: {}, AvgLoss: {}'.format(
                    batch_id,
                    np.around(loss.data.cpu().numpy().tolist(), 4),
                ...
                with torch.no_grad():
                    ...
                    for batch_dev_id in range(no_batches['valid']):
                        subgraphs, all_nodes, labels =\
                            ft_batch_input['valid'][batch_dev_id]
                    ...
                    if loss_dev_avg < loss_dev_min:
                        ...
                        fmodel = open(os.path.join(
                            ft_out_dir, 'finetune_'+str(epoch)+'.model'), 'wb')
    ...
    pred_data = []
    true_data = [] 
    path_metrics_pretrained = PathMetrics(pretrained_node_embedding_tensor, attr_graph.G, ent2id,\
        args.d_model, args.ft_d_ff, args.ft_layer, args.ft_drop_rate, 
        attr_graph, args.ft_input_option, args.n_layers)
    path_metrics_graphbert_ft = PathMetrics(None, attr_graph.G, ent2id,\
        args.d_model, args.ft_d_ff, args.ft_layer, args.ft_drop_rate, 
        attr_graph, args.ft_input_option, args.n_layers)
    with torch.no_grad():
        att_weights = {}
        ...
        for batch_test_id in range(no_batches['test']):
            ...
            subgraphs, all_nodes, labels  = \
                ft_batch_input['test'][batch_test_id]
            masked_pos = torch.randn(args.batch_size, 2)
            masked_nodes = Variable(torch.LongTensor([[] for ii in range(args.ft_batch_size)]))
            output,layer_output, att_output = graph_bert(subgraphs, all_nodes, masked_pos, masked_nodes)
        ...
        data = json.dump(path_dict, open(outdir+'path_metrics_pretrained.json', 'w'), indent=4)
        path_dict = path_metrics_graphbert_ft.finalize()
        data = json.dump(path_dict, open(outdir+'path_metrics_graphbert_ft.json', 'w'), indent=4)
